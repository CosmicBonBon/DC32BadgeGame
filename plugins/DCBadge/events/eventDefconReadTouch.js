const id = "EVENT_DEFCON_READ_TOUCH";
...
const compile = (input, helpers) => {
  ...
  const touchValueRef = _declareLocal("touch_value", 1, true);
  ...
  const axisLabel = fields
    .find((f) => f.key === "axis")
    .options.find((o) => o[0] === input.axis)?.[1];
  _addComment(`Def Con Read Touch Axis ${axisLabel}`);
...
module.exports = {
  id,
  name: "Def Con Read Touch Controller",
  groups,
  subGroups,
  fields,
  compile,
