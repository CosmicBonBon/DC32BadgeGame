const id = "EVENT_DEFCON_READ_ADC";
...
const compile = (input, helpers) => {
  ...
  const adcValueRef = _declareLocal("adc_value", 1, true);
  ...
  console.log(
    fields
      .find((f) => f.key === "value")
      .options.find((o) => o[0] === input.value)?.[1]
  );
  const valueLabel = fields
    .find((f) => f.key === "value")
    .options.find((o) => o[0] === input.value)?.[1];
  _addComment(`Def Con Read ADC Value ${valueLabel}`);
